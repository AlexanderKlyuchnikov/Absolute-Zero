...
local root = (...):match("(.+)/.-")
local config = JustConfig:fromFile(root .. "/8key.config.lua")
...
playfield:addGuidelines({
	y = {0, 0, 0, 0, 0, 0, 0, 0, 0},
	w = {2, 0, 0, 0, 0, 0, 0, 0, 2},
	h = {noteskin.unit, 0, 0, 0, 0, 0, 0, 0, noteskin.unit},
...
if config:get("middleline") then
	playfield:addGuidelines({
		y = {0, 0, 0, 0, 0, 0, 0, 0, 0},
		w = {0, 0, 0, 0, 1, 0, 0, 0, 0},
		h = {0, 0, 0, 0, noteskin.hitposition, 0, 0, 0, 0},
		image = "Other/Pixel.png",
		color = {0, 0.3, 0.4, 1},
		both = false,
		mode = "default",
...
if config:get("hiterror") then
	functions.addHitError(playfield, cs * 8)
end
